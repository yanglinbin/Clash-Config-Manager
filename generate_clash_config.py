...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('clash_generator.log', encoding='utf-8'),
...
class ClashConfigGenerator:
    def __init__(self, config_file='config.ini'):
        ...
        # 从配置文件获取规则文件路径
        self.rules_file = self.config.get('files', 'rules_config', fallback='rules.yaml')
    ...
    def generate_proxy_providers_config(self, providers: Dict[str, str]) -> Dict[str, Any]:
        """生成 proxy-providers 配置"""
        proxy_providers = {}
        test_url = self.config.get('clash', 'test_url', fallback='http://connectivitycheck.gstatic.com/generate_204')
    ...
    def generate_auto_groups(self, providers: Dict[str, str], regions: Dict[str, Dict[str, any]]) -> List[Dict[str, Any]]:
        """生成自动选择组"""
        auto_groups = []
        test_url = self.config.get('clash', 'test_url', fallback='http://connectivitycheck.gstatic.com/generate_204')
    ...
    def generate_main_proxy_groups(self, providers: Dict[str, str], regions: Dict[str, Dict[str, any]]) -> List[Dict[str, Any]]:
        """生成主要代理组"""
    ...
    def generate_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            'external-controller': self.config.get('clash', 'external_controller', fallback=':9090'),
    ...
    def save_config(self, config: Dict[str, Any], output_file: str = 'clash_profile.yaml'):
        """保存配置到文件"""
        try:
