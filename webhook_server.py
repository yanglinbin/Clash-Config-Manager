...
class WebhookServer:
    ...
    def regenerate_config(self):
        ...
        try:
            result = subprocess.run(
                [sys.executable, 'generate_clash_config.py'],
                capture_output=True,
                text=True,
                timeout=60
            )
    ...
    def handle_push_event(self, payload):
        ...
        try:
            ...
            # 检查是否有相关文件变更
            relevant_files = ['config.ini', 'generate_clash_config.py', 'webhook_server.py']
...
def status():
    ...
    status_info = {
        ...
        'status': 'running',
        'timestamp': datetime.now().isoformat(),
        'last_update': webhook_server.last_update.isoformat() if webhook_server.last_update else None,
        'config_file': webhook_server.config_file,
        'webhook_secret_configured': bool(webhook_server.webhook_secret)
    }
    
    # 检查配置文件是否存在
    config_exists = Path('clash_profile.yaml').exists()
    if config_exists:
        config_stat = Path('clash_profile.yaml').stat()
        status_info['config_file_exists'] = True
        status_info['config_file_size'] = config_stat.st_size
        status_info['config_file_modified'] = datetime.fromtimestamp(config_stat.st_mtime).isoformat()
    else:
...
def index():
    ...
    return render_template_string(html_template,
        ...
        last_update=webhook_server.last_update.strftime('%Y-%m-%d %H:%M:%S') if webhook_server.last_update else '从未更新',
        config_exists='✅ 存在' if Path('clash_profile.yaml').exists() else '❌ 不存在',
        webhook_configured='✅ 已配置' if webhook_server.webhook_secret else '❌ 未配置'
    )
