...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('update_service.log', encoding='utf-8'),
...
class UpdateService:
    def __init__(self, config_file='config.ini'):
        ...
        self.update_interval = self.config.getint('server', 'update_interval', fallback=3600)
    ...
    def check_config_file_age(self) -> bool:
        """检查配置文件是否需要更新"""
        config_file = Path('clash_profile.yaml')
    ...
    def get_status(self) -> dict:
        ...
        status = {
            ...
            'config_file_modified': datetime.fromtimestamp(config_file.stat().st_mtime).isoformat() if config_file.exists() else None,
            'next_check': (datetime.now() + timedelta(seconds=self.update_interval)).isoformat()
        }
